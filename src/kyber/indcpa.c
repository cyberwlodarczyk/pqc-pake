#include <stddef.h>
#include <stdint.h>
#include <string.h>
#include "params.h"
#include "indcpa.h"
#include "polyvec.h"
#include "poly.h"
#include "ntt.h"
#include "symmetric.h"
#include "randombytes.h"

static void pack_pk(
    uint8_t r[KYBER_INDCPA_LEN_PUBLIC_KEY],
    KYBER_polyvec *pk,
    const uint8_t seed[KYBER_LEN_SEED])
{
    KYBER_polyvec_tobytes(r, pk);
    memcpy(r + KYBER_LEN_POLYVEC, seed, KYBER_LEN_SEED);
}

static void unpack_pk(
    KYBER_polyvec *pk,
    uint8_t seed[KYBER_LEN_SEED],
    const uint8_t packedpk[KYBER_INDCPA_LEN_PUBLIC_KEY])
{
    KYBER_polyvec_frombytes(pk, packedpk);
    memcpy(seed, packedpk + KYBER_LEN_POLYVEC, KYBER_LEN_SEED);
}

static void pack_sk(uint8_t r[KYBER_INDCPA_LEN_SECRET_KEY], KYBER_polyvec *sk)
{
    KYBER_polyvec_tobytes(r, sk);
}

static void unpack_sk(
    KYBER_polyvec *sk,
    const uint8_t packedsk[KYBER_INDCPA_LEN_SECRET_KEY])
{
    KYBER_polyvec_frombytes(sk, packedsk);
}

static void pack_ciphertext(
    uint8_t r[KYBER_INDCPA_LEN_CIPHERTEXT],
    KYBER_polyvec *b,
    KYBER_poly *v)
{
    KYBER_polyvec_compress(r, b);
    KYBER_poly_compress(r + KYBER_LEN_POLYVEC_COMPRESSED, v);
}

static void unpack_ciphertext(
    KYBER_polyvec *b,
    KYBER_poly *v,
    const uint8_t c[KYBER_INDCPA_LEN_CIPHERTEXT])
{
    KYBER_polyvec_decompress(b, c);
    KYBER_poly_decompress(v, c + KYBER_LEN_POLYVEC_COMPRESSED);
}

static unsigned int rej_uniform(
    int16_t *r,
    unsigned int len,
    const uint8_t *buf,
    unsigned int buflen)
{
    unsigned int ctr, pos;
    uint16_t val0, val1;
    ctr = pos = 0;
    while (ctr < len && pos + 3 <= buflen)
    {
        val0 = ((buf[pos + 0] >> 0) | ((uint16_t)buf[pos + 1] << 8)) & 0xFFF;
        val1 = ((buf[pos + 1] >> 4) | ((uint16_t)buf[pos + 2] << 4)) & 0xFFF;
        pos += 3;
        if (val0 < KYBER_Q)
            r[ctr++] = val0;
        if (ctr < len && val1 < KYBER_Q)
            r[ctr++] = val1;
    }
    return ctr;
}

#define GEN_MATRIX_NBLOCKS ((12 * KYBER_N / 8 * (1 << 12) / KYBER_Q + XOF_BLOCKBYTES) / XOF_BLOCKBYTES)

void KYBER_gen_matrix(KYBER_polyvec *a, const uint8_t seed[KYBER_LEN_SEED], int transposed)
{
    unsigned int ctr, i, j;
    unsigned int buflen;
    uint8_t buf[GEN_MATRIX_NBLOCKS * XOF_BLOCKBYTES];
    xof_state state;
    for (i = 0; i < KYBER_K; i++)
    {
        for (j = 0; j < KYBER_K; j++)
        {
            if (transposed)
                KYBER_xof_absorb(&state, seed, i, j);
            else
                KYBER_xof_absorb(&state, seed, j, i);

            KYBER_xof_squeezeblocks(buf, GEN_MATRIX_NBLOCKS, &state);
            buflen = GEN_MATRIX_NBLOCKS * XOF_BLOCKBYTES;
            ctr = rej_uniform(a[i].vec[j].coeffs, KYBER_N, buf, buflen);

            while (ctr < KYBER_N)
            {
                KYBER_xof_squeezeblocks(buf, 1, &state);
                buflen = XOF_BLOCKBYTES;
                ctr += rej_uniform(a[i].vec[j].coeffs + ctr, KYBER_N - ctr, buf, buflen);
            }
        }
    }
}

#define KYBER_gen_a(A, B) KYBER_gen_matrix(A, B, 0)
#define KYBER_gen_at(A, B) KYBER_gen_matrix(A, B, 1)

void KYBER_indcpa_keygen_derand(
    uint8_t pk[KYBER_INDCPA_LEN_PUBLIC_KEY],
    uint8_t sk[KYBER_INDCPA_LEN_SECRET_KEY],
    const uint8_t coins[KYBER_LEN_SEED])
{
    unsigned int i;
    uint8_t buf[2 * KYBER_LEN_SEED];
    const uint8_t *publicseed = buf;
    const uint8_t *noiseseed = buf + KYBER_LEN_SEED;
    uint8_t nonce = 0;
    KYBER_polyvec a[KYBER_K], e, pkpv, skpv;
    memcpy(buf, coins, KYBER_LEN_SEED);
    buf[KYBER_LEN_SEED] = KYBER_K;
    KYBER_hash_g(buf, buf, KYBER_LEN_SEED + 1);
    KYBER_gen_a(a, publicseed);
    for (i = 0; i < KYBER_K; i++)
        KYBER_poly_get_noise_eta1(&skpv.vec[i], noiseseed, nonce++);
    for (i = 0; i < KYBER_K; i++)
        KYBER_poly_get_noise_eta1(&e.vec[i], noiseseed, nonce++);
    KYBER_polyvec_ntt(&skpv);
    KYBER_polyvec_ntt(&e);
    for (i = 0; i < KYBER_K; i++)
    {
        KYBER_polyvec_basemul_acc_montgomery(&pkpv.vec[i], &a[i], &skpv);
        KYBER_poly_tomont(&pkpv.vec[i]);
    }
    KYBER_polyvec_add(&pkpv, &pkpv, &e);
    KYBER_polyvec_reduce(&pkpv);
    pack_sk(sk, &skpv);
    pack_pk(pk, &pkpv, publicseed);
}

void KYBER_indcpa_encaps(
    uint8_t c[KYBER_INDCPA_LEN_CIPHERTEXT],
    const uint8_t m[KYBER_INDCPA_LEN_MSG],
    const uint8_t pk[KYBER_INDCPA_LEN_PUBLIC_KEY],
    const uint8_t coins[KYBER_LEN_SEED])
{
    KYBER_polyvec sp, pkpv, ep, at[KYBER_K], b;
    KYBER_poly v, k, epp;
    uint8_t seed[KYBER_LEN_SEED];
    uint8_t nonce = 0;
    unpack_pk(&pkpv, seed, pk);
    KYBER_poly_frommsg(&k, m);
    KYBER_gen_at(at, seed);
    for (int i = 0; i < KYBER_K; i++)
    {
        KYBER_poly_get_noise_eta1(sp.vec + i, coins, nonce++);
    }
    for (int i = 0; i < KYBER_K; i++)
    {
        KYBER_poly_get_noise_eta2(ep.vec + i, coins, nonce++);
    }
    KYBER_poly_get_noise_eta2(&epp, coins, nonce++);
    KYBER_polyvec_ntt(&sp);
    for (int i = 0; i < KYBER_K; i++)
    {
        KYBER_polyvec_basemul_acc_montgomery(&b.vec[i], &at[i], &sp);
    }
    KYBER_polyvec_basemul_acc_montgomery(&v, &pkpv, &sp);
    KYBER_polyvec_invntt_tomont(&b);
    KYBER_poly_invntt_tomont(&v);
    KYBER_polyvec_add(&b, &b, &ep);
    KYBER_poly_add(&v, &v, &epp);
    KYBER_poly_add(&v, &v, &k);
    KYBER_polyvec_reduce(&b);
    KYBER_poly_reduce(&v);
    pack_ciphertext(c, &b, &v);
}

void KYBER_indcpa_decaps(
    uint8_t m[KYBER_INDCPA_LEN_MSG],
    const uint8_t c[KYBER_INDCPA_LEN_CIPHERTEXT],
    const uint8_t sk[KYBER_INDCPA_LEN_SECRET_KEY])
{
    KYBER_polyvec b, skpv;
    KYBER_poly v, mp;
    unpack_ciphertext(&b, &v, c);
    unpack_sk(&skpv, sk);
    KYBER_polyvec_ntt(&b);
    KYBER_polyvec_basemul_acc_montgomery(&mp, &skpv, &b);
    KYBER_poly_invntt_tomont(&mp);
    KYBER_poly_sub(&mp, &v, &mp);
    KYBER_poly_reduce(&mp);
    KYBER_poly_tomsg(m, &mp);
}
